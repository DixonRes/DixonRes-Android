...
int main(int argc, char *argv[]) {
    // Disable output buffering for Android compatibility
    setvbuf(stdout, NULL, _IONBF, 0);
    setvbuf(stderr, NULL, _IONBF, 0);
    
    fprintf(stderr, "[MAIN-STEP] 1: Entered main()\n");
    fflush(stderr);
    
    clock_t start_time = clock();
    const char *prog_name = display_prog_name(argv[0]);

    fprintf(stderr, "[MAIN-STEP] 2: Got prog_name\n");
    fflush(stderr);
    
    // Debug: Print program start information
    ...
    fprintf(stderr, "[DEBUG] Number of arguments: %d\n", argc);
    for (int i = 0; i < argc; i++) {
        fprintf(stderr, "[DEBUG] argv[%d]: %s\n", i, argv[i]);
    }
    fprintf(stderr, "[MAIN-STEP] 3: Printed args\n");
    fflush(stderr);
    
    char *cwd = getcwd(NULL, 0);
    fprintf(stderr, "[DEBUG] Current directory: %s\n", cwd);
    free(cwd);
    ...
    fprintf(stderr, "[DEBUG] ========================================\n");
    fprintf(stderr, "[MAIN-STEP] 4: Printed env\n");
    fflush(stderr);

    if (argc == 1) { 
        fprintf(stderr, "[DEBUG] No arguments provided, printing usage\n");
        print_usage(prog_name); 
        return 0; 
    }
    fprintf(stderr, "[MAIN-STEP] 5: Checked argc\n");
    fflush(stderr);
    
    fprintf(stderr, "[DEBUG] Arguments parsed, continuing execution\n");
    fprintf(stderr, "[DEBUG] About to parse leading flags\n");
    fflush(stderr);

    /* ---- parse leading flags ---- */
    fprintf(stderr, "[MAIN-STEP] 6: Starting to parse leading flags\n");
    fflush(stderr);
    ...
    fprintf(stderr, "[MAIN-STEP] 7: Variables declared, starting loop\n");
    fflush(stderr);
    ...
    fprintf(stderr, "[MAIN-STEP] 8: Done parsing leading flags\n");
    fprintf(stderr, "[DEBUG] silent_mode=%d, solve_mode=%d, comp_mode=%d, rand_mode=%d, ideal_mode=%d\n",
           silent_mode, solve_mode, comp_mode, rand_mode, ideal_mode);
    fprintf(stderr, "[DEBUG] effective_argc=%d\n", effective_argc);
    fprintf(stderr, "[MAIN-STEP] 9: About to check help\n");
    fflush(stderr);
    ...
    /* ---- version banner ---- */
    fprintf(stderr, "[MAIN-STEP] 10: About to print version banner\n");
    fflush(stderr);
    if (!silent_mode) {
        printf("=================================================\n");
        printf("DixonRes v%s\n", PROGRAM_VERSION);
    ...
    fprintf(stderr, "[MAIN-STEP] 11: Done printing version banner\n");
    fflush(stderr);
    ...
    /* ---- input variables ---- */
    fprintf(stderr, "[MAIN-STEP] 12: About to declare input variables\n");
    fflush(stderr);
    ...
    fprintf(stderr, "[MAIN-STEP] 13: About to determine input mode\n");
    fflush(stderr);
    ...
    fprintf(stderr, "[MAIN-STEP] 14: Done determining input mode\n");
    fprintf(stderr, "[DEBUG] polys_str: %s\n", polys_str ? polys_str : "(null)");
    fprintf(stderr, "[DEBUG] vars_str: %s\n", vars_str ? vars_str : "(null)");
    fprintf(stderr, "[DEBUG] field_str: %s\n", field_str ? field_str : "(null)");
    fprintf(stderr, "[MAIN-STEP] 15: About to parse field size\n");
    fflush(stderr);

    /* ---- parse field size ---- */
    fprintf(stderr, "[DEBUG] About to parse field size: %s\n", field_str);
    fflush(stderr);
    ...
    fprintf(stderr, "[MAIN-STEP] 16: Done parsing field size\n");
    fflush(stderr);
    ...
    fprintf(stderr, "[DEBUG] rational_mode=%d, large_prime_mode=%d, power=%lu\n",
           rational_mode, large_prime_mode, power);
    fprintf(stderr, "[MAIN-STEP] 17: About to initialize finite field\n");
    fflush(stderr);
    ...
    fprintf(stderr, "[DEBUG] About to initialize finite field\n");
    fflush(stderr);
    
    fq_nmod_ctx_t ctx;

    if (!rational_mode && !large_prime_mode && power > 1 && field_poly_str) {
        fprintf(stderr, "[DEBUG] Initializing with custom field polynomial\n");
        fflush(stderr);
        ...
        fprintf(stderr, "[DEBUG] Calling fq_nmod_ctx_init_modulus\n");
        fflush(stderr);
        fq_nmod_ctx_init_modulus(ctx, modulus, var_name);
        ctx_initialized = 1;
        nmod_poly_clear(modulus);
        fprintf(stderr, "[DEBUG] fq_nmod_ctx_init_modulus completed\n");
        fflush(stderr);

    } else if (!rational_mode && !large_prime_mode) {
        fprintf(stderr, "[DEBUG] Initializing finite field with fq_nmod_ctx_init\n");
        fprintf(stderr, "[DEBUG] p_fmpz = %ld, power = %lu\n", fmpz_get_ui(p_fmpz), power);
        fflush(stderr);
        fq_nmod_ctx_init(ctx, p_fmpz, power, "t");
        ctx_initialized = 1;
        fprintf(stderr, "[DEBUG] fq_nmod_ctx_init completed\n");
        fprintf(stderr, "[DEBUG] ctx = %p\n", (void*)ctx);
        fprintf(stderr, "[DEBUG] ctx->mod.n = %lu\n", ctx->mod.n);
        fprintf(stderr, "[DEBUG] ctx->modulus = %p\n", (void*)ctx->modulus);
        fprintf(stderr, "[DEBUG] ctx->modulus->length = %ld\n", (slong)ctx->modulus[0].length);
        fprintf(stderr, "[DEBUG] fq_nmod_ctx_degree = %ld\n", fq_nmod_ctx_degree(ctx));
        fflush(stderr);
        ...
        fprintf(stderr, "[DEBUG] Skipping finite field initialization (rational or large prime mode)\n");
        fflush(stderr);
    }

    fprintf(stderr, "[DEBUG] Finished finite field initialization\n");
    fflush(stderr);
    ...
    fprintf(stderr, "[DEBUG] Executing mode: comp_mode=%d, solve_mode=%d, ideal_mode=%d\n", comp_mode, solve_mode, ideal_mode);
    fprintf(stderr, "[DEBUG] Rational mode: %d, Large prime mode: %d\n", rational_mode, large_prime_mode);
    fprintf(stderr, "[DEBUG] Field size: %s\n", field_str);
    fprintf(stderr, "[DEBUG] Polynomials: %s\n", polys_str);
    fprintf(stderr, "[DEBUG] Variables: %s\n", vars_str);
    fprintf(stderr, "[MAIN-STEP] 18: About to execute selected mode\n");
    fflush(stderr);
    ...
    if (comp_mode) {
        ...
        fprintf(stderr, "[DEBUG] Entering solver mode\n");
        fflush(stderr);
        if (!silent_mode) {
        ...
        if (rational_mode) {
            fprintf(stderr, "[DEBUG] Entering rational solver mode\n");
            fflush(stderr);
            ...
            fprintf(stderr, "[DEBUG] Calling solve_rational_polynomial_system_string\n");
            fflush(stderr);
            rational_solutions = solve_rational_polynomial_system_string(polys_str);
            fprintf(stderr, "[DEBUG] solve_rational_polynomial_system_string returned\n");
            fflush(stderr);
            ...
            fprintf(stderr, "[DEBUG] Entering finite field solver mode\n");
            fflush(stderr);
            ...
            fprintf(stderr, "[DEBUG] Calling solve_polynomial_system_string\n");
            fflush(stderr);
            solutions = solve_polynomial_system_string(polys_str, ctx);
            fprintf(stderr, "[DEBUG] solve_polynomial_system_string returned\n");
            fflush(stderr);
        ...
        fprintf(stderr, "[DEBUG] Entering ideal reduction mode\n");
        fflush(stderr);
        if (!silent_mode) {
        ...
        fprintf(stderr, "[DEBUG] Calling dixon_with_ideal_reduction_str\n");
        fflush(stderr);
        result = dixon_with_ideal_reduction_str(polys_str, vars_str, ideal_str, ctx);
        fprintf(stderr, "[DEBUG] dixon_with_ideal_reduction_str returned: %s\n", result ? result : "NULL");
        fflush(stderr);

    } else {
        /* ---- Basic Dixon resultant ---- */
        fprintf(stderr, "[DEBUG] Entering basic Dixon resultant mode\n");
        fflush(stderr);
        if (!silent_mode) {
            int poly_count = count_comma_separated_items(polys_str);
            int var_count  = count_comma_separated_items(vars_str);
        ...
        // Check for simple case: polynomials are just variables (applies to both rational and finite field modes)
        slong num_polys, num_vars;
        fprintf(stderr, "[DEBUG] Splitting polynomials and variables\n");
        fflush(stderr);
        char **poly_array = split_string(polys_str, &num_polys);
        char **vars_array = split_string(vars_str, &num_vars);
        fprintf(stderr, "[DEBUG] Split completed: num_polys=%ld, num_vars=%ld\n", num_polys, num_vars);
        fflush(stderr);
        
        int is_simple_case = 1;
        if (num_polys != num_vars + 1) {
            fprintf(stderr, "[DEBUG] Not simple case: num_polys != num_vars + 1\n");
            is_simple_case = 0;
        } else {
            fprintf(stderr, "[DEBUG] Checking if polynomials are just variables\n");
            fflush(stderr);
            for (slong i = 0; i < num_polys; i++) {
                char *trimmed = trim(poly_array[i]);
                if (strlen(trimmed) == 0) {
                    fprintf(stderr, "[DEBUG] Not simple case: empty polynomial\n");
                    is_simple_case = 0;
                    break;
                }
                // Check if the polynomial is just a single variable
                int has_non_alpha = 0;
                for (size_t j = 0; j < strlen(trimmed); j++) {
                    if (!isalpha(trimmed[j]) && trimmed[j] != '_') {
                        has_non_alpha = 1;
                        break;
                    }
                }
                if (has_non_alpha) {
                    fprintf(stderr, "[DEBUG] Not simple case: polynomial has non-alpha characters\n");
                    is_simple_case = 0;
                    break;
                }
            }
        ...
        if (is_simple_case) {
            // For simple case, return the last polynomial as the result
            fprintf(stderr, "[DEBUG] Simple case detected: polynomials are just variables\n");
            fflush(stderr);
            if (num_polys > 0) {
                result = strdup(poly_array[num_polys - 1]);
                fprintf(stderr, "[DEBUG] Simple case result: %s\n", result);
            } else {
                result = strdup("0");
                fprintf(stderr, "[DEBUG] Simple case result: 0\n");
            }
            fflush(stderr);
        } else {
            if (rational_mode) {
                fprintf(stderr, "[DEBUG] Entering rational mode\n");
                fflush(stderr);
                if (num_polys != num_vars + 1) {
                    if (!silent_mode) {
                    ...
                    result = strdup("0");
                    fprintf(stderr, "[DEBUG] Rational mode error: incorrect number of polynomials\n");
                } else {
                    fprintf(stderr, "[DEBUG] Calling dixon_str_rational...\n");
                    fflush(stderr);
                    if (output_filename) {
                        result = dixon_str_rational_with_file(polys_str, vars_str, output_filename);
                        fprintf(stderr, "[DEBUG] dixon_str_rational_with_file returned\n");
                    } else {
                        result = dixon_str_rational(polys_str, vars_str);
                        fprintf(stderr, "[DEBUG] dixon_str_rational returned\n");
                    }
                    fprintf(stderr, "[DEBUG] Rational mode result: %s\n", result ? result : "NULL");
                }
                fflush(stderr);
            } else if (large_prime_mode) {
                fprintf(stderr, "[DEBUG] Entering large prime mode\n");
                fflush(stderr);
                result = dixon_str_large_prime(polys_str, vars_str, p_fmpz);
                fprintf(stderr, "[DEBUG] dixon_str_large_prime returned: %s\n", result ? result : "NULL");
                fflush(stderr);
            } else {
                fprintf(stderr, "[DEBUG] Entering finite field mode\n");
                fprintf(stderr, "[DEBUG] Calling dixon_str with polys_str: %s, vars_str: %s\n", polys_str, vars_str);
                fflush(stderr);
                result = dixon_str(polys_str, vars_str, ctx);
                fprintf(stderr, "[DEBUG] dixon_str returned: %s\n", result ? result : "NULL");
                fflush(stderr);
            }
        }
        
        fprintf(stderr, "[DEBUG] Freeing split strings\n");
        fflush(stderr);
        free_split_strings(poly_array, num_polys);
        free_split_strings(vars_array, num_vars);
        fprintf(stderr, "[DEBUG] Freeing completed\n");
        fflush(stderr);
