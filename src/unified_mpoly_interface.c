    ...
    switch (field_ctx->field_id) {
        case FIELD_ID_NMOD:
            {
                ...
                fprintf(stderr, "[DEBUG unified_mpoly_ctx_init] modulus=%lu, nvars=%ld, ord=%d\n", modulus, nvars, ord);
                fflush(stderr);
                nmod_mpoly_ctx_init(GET_NMOD_CTX(ctx), nvars, ord, modulus);
                fprintf(stderr, "[DEBUG unified_mpoly_ctx_init] nmod_mpoly_ctx_init done\n");
                fflush(stderr);
                nmod_mpoly_ctx_struct *nmod_ctx = GET_NMOD_CTX(ctx);
                fprintf(stderr, "[DEBUG unified_mpoly_ctx_init] nmod_ctx=%p, &nmod_ctx->minfo=%p, nmod_ctx->minfo.nvars=%ld\n", 
        ...
        case FIELD_ID_FQ_ZECH:
            ...
            if (field_ctx->ctx.zech_ctx) {
                ...
                mp_limb_t p = fq_nmod_ctx_prime(field_ctx->ctx.zech_ctx->fq_nmod_ctx);
    ...
    switch (ctx->field_ctx->field_id) {
        ...
        case FIELD_ID_FQ_ZECH:
            if (ctx->field_ctx->ctx.zech_ctx) {
                mp_limb_t p = fq_nmod_ctx_prime(ctx->field_ctx->ctx.zech_ctx->fq_nmod_ctx);
