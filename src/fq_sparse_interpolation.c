...
void poly_mat_det_2x2(nmod_mpoly_t det, const poly_mat_t* mat, 
    ...
    // Fix: Ensure det has valid exps field
    if (det->exps == NULL && det->length > 0) {
        // This is a constant polynomial, set exps to zero
        det->bits = MPOLY_MIN_BITS;
        det->exps = (ulong *) flint_calloc(det->length, (det->bits * ctx->minfo->nvars + FLINT_BITS - 1) / FLINT_BITS * sizeof(ulong));
    }
