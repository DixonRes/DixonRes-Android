...
static void parse_identifier(parser_state_t *state) {
    ...
    if (state->generator_name && strcmp(state->current.str, state->generator_name) == 0) {
        ...
        fprintf(stderr, "[DEBUG] Generator: %s\n", state->current.str);
    } else {
        state->current.type = TOK_VARIABLE;
        fprintf(stderr, "[DEBUG] Identifier: %s\n", state->current.str);
    }
}
...
char* get_generator_name(const fq_nmod_ctx_t ctx) {
    fprintf(stderr, "[DEBUG] get_generator_name called with ctx=%p\n", (void*)ctx);
    if (!ctx) {
        fprintf(stderr, "[DEBUG] get_generator_name: ctx is NULL, returning default\n");
        return strdup("t");
    }
    fprintf(stderr, "[DEBUG] get_generator_name: ctx->var=%p\n", (void*)ctx->var);
    if (!ctx->var) {
        fprintf(stderr, "[DEBUG] get_generator_name: ctx->var is NULL, returning default\n");
        return strdup("t");
    }
    fprintf(stderr, "[DEBUG] get_generator_name: ctx->var=%s\n", ctx->var);
...
char* compute_dixon_internal_with_file(const char **poly_strings, slong npoly_strings,
    ...
    fprintf(stderr, "[DEBUG] compute_dixon_internal_with_file: ENTERED\n");
    fflush(stderr);
    
    // Check if ctx is valid
    if (!ctx) {
        fprintf(stderr, "[DEBUG] ERROR: ctx is NULL\n");
        *remaining_vars = NULL;
        *num_remaining = 0;
        return strdup("0");
    }
    
    fprintf(stderr, "[DEBUG] ctx pointer: %p\n", (void*)ctx);
    fflush(stderr);
    ...
    // Get generator name
    fprintf(stderr, "[DEBUG] About to call get_generator_name\n");
    fflush(stderr);
    char *gen_name = get_generator_name(ctx);
    fprintf(stderr, "[DEBUG] compute_dixon_internal_with_file: got generator name: %s\n", gen_name ? gen_name : "(null)");
    fflush(stderr);
    ...
    fprintf(stderr, "[DEBUG] compute_dixon_internal_with_file: initialized parser state\n");
    
    // First pass: identify parameters
    for (slong i = 0; i < npoly_strings; i++) {
        fprintf(stderr, "[DEBUG] compute_dixon_internal_with_file: first pass poly %ld\n", i);
        fq_mvpoly_t temp;
    ...
    fprintf(stderr, "[DEBUG] compute_dixon_internal_with_file: first pass complete, npars = %ld\n", state.npars);
    ...
    *num_remaining = state.npars;
    ...
    fprintf(stderr, "[DEBUG] compute_dixon_internal_with_file: saved remaining variables\n");
    
    // Second pass: parse polynomials
    fq_mvpoly_t *polys = (fq_mvpoly_t*) malloc(npoly_strings * sizeof(fq_mvpoly_t));
    
    for (slong i = 0; i < npoly_strings; i++) {
        fprintf(stderr, "[DEBUG] compute_dixon_internal_with_file: second pass poly %ld\n", i);
    ...
    fprintf(stderr, "[DEBUG] compute_dixon_internal_with_file: second pass complete\n");
    
    // Compute Dixon resultant with original names
    fprintf(stderr, "[DEBUG] compute_dixon_internal_with_file: calling fq_dixon_resultant_with_names\n");
    ...
    if (dixon_result_poly.nterms == 0) {
        ...
        fprintf(stderr, "[DEBUG] compute_dixon_internal_with_file: calling fq_mvpoly_to_string\n");
        result_string = fq_mvpoly_to_string(&dixon_result_poly, state.par_names, gen_name);
        fprintf(stderr, "[DEBUG] compute_dixon_internal_with_file: fq_mvpoly_to_string returned\n");
    }
    
    // Cleanup
    fprintf(stderr, "[DEBUG] compute_dixon_internal_with_file: starting cleanup\n");
    ...
    fprintf(stderr, "[DEBUG] compute_dixon_internal_with_file: end\n");
...
static char *qq_reconstruct_from_modular_dixon_with_file(const char *poly_string,
    ...
    for (slong i = 0; i < num_primes; i++) {
        ...
        // Debug: Check ctx contents right after creation
        fprintf(stderr, "[DEBUG] After fq_nmod_ctx_init: ctx=%p\n", (void*)ctx);
...
char* dixon_with_file(const char **poly_strings, slong num_polys, const char **elim_vars, slong num_elim_vars, const fq_nmod_ctx_t ctx, FILE *fp_file, int print_to_stdout) {
    fprintf(stderr, "[DEBUG] dixon_with_file called with %ld polynomials, %ld elimination variables\n", num_polys, num_elim_vars);
    for (slong i = 0; i < num_polys; i++) {
        fprintf(stderr, "[DEBUG] Poly %ld: %s\n", i, poly_strings[i]);
    }
    for (slong i = 0; i < num_elim_vars; i++) {
        fprintf(stderr, "[DEBUG] Elim var %ld: %s\n", i, elim_vars[i]);
    }
    clock_t start = clock();
    ...
    fprintf(stderr, "[DEBUG] Checking parameters:\n");
    fprintf(stderr, "[DEBUG]   poly_strings: %p\n", (void*)poly_strings);
    fprintf(stderr, "[DEBUG]   num_polys: %ld\n", num_polys);
    fprintf(stderr, "[DEBUG]   elim_vars: %p\n", (void*)elim_vars);
    fprintf(stderr, "[DEBUG]   num_elim_vars: %ld\n", num_elim_vars);
    fprintf(stderr, "[DEBUG]   ctx: %p\n", (void*)ctx);
    fprintf(stderr, "[DEBUG]   remaining_vars: %p\n", (void*)&remaining_vars);
    fprintf(stderr, "[DEBUG]   num_remaining: %p\n", (void*)&num_remaining);
    fflush(stderr);
    
    char *result = compute_dixon_internal_with_file(poly_strings, num_polys, elim_vars, num_elim_vars, ctx, &remaining_vars, &num_remaining, fp_file, print_to_stdout);
    fprintf(stderr, "[DEBUG] compute_dixon_internal_with_file returned: %s\n", result ? result : "(null)");
    fflush(stderr);
    
    // Cleanup remaining vars
    if (remaining_vars) {
        fprintf(stderr, "[DEBUG] Cleaning up remaining vars: %ld variables\n", num_remaining);
    ...
    fprintf(stderr, "[DEBUG] dixon_with_file completed in %.3f seconds\n", elapsed);
    return result;
}
...
char* dixon_str_with_file(const char *poly_string, const char *vars_string, const fq_nmod_ctx_t ctx, FILE *fp_file, int print_to_stdout) {
    fprintf(stderr, "[DEBUG] dixon_str_with_file called with poly_string: %s, vars_string: %s\n", poly_string, vars_string);
    slong num_polys, num_vars;
    char **poly_array = split_string(poly_string, &num_polys);
    char **vars_array = split_string(vars_string, &num_vars);
    fprintf(stderr, "[DEBUG] Split strings: %ld polynomials, %ld variables\n", num_polys, num_vars);
    for (slong i = 0; i < num_polys; i++) {
        fprintf(stderr, "[DEBUG] Poly %ld: %s\n", i, poly_array[i]);
    }
    for (slong i = 0; i < num_vars; i++) {
        fprintf(stderr, "[DEBUG] Var %ld: %s\n", i, vars_array[i]);
    }
    char *result = NULL;
    ...
    fprintf(stderr, "[DEBUG] dixon_str_with_file completed\n");
    return result;
}
