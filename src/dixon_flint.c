...
void perform_fq_matrix_row_operations_mvpoly(fq_mvpoly_t ***new_matrix, fq_mvpoly_t ***original_matrix,
                                                   slong nvars, slong npars) {
    fprintf(stderr, "[DEBUG] perform_fq_matrix_row_operations_mvpoly: START nvars=%ld, npars=%ld\n", nvars, npars);
    fflush(stderr);
    
    slong n = nvars + 1;
    fprintf(stderr, "[DEBUG] Matrix size: %ld x %ld\n", n, n);
    fflush(stderr);
    
    fprintf(stderr, "[DEBUG] Allocating matrix...\n");
    fflush(stderr);
    *new_matrix = (fq_mvpoly_t**) flint_malloc(n * sizeof(fq_mvpoly_t*));
    for (slong i = 0; i < n; i++) {
        (*new_matrix)[i] = (fq_mvpoly_t*) flint_malloc(n * sizeof(fq_mvpoly_t));
    }
    fprintf(stderr, "[DEBUG] Matrix allocated\n");
    fflush(stderr);
    
    fprintf(stderr, "[DEBUG] Copying first row...\n");
    fflush(stderr);
    for (slong j = 0; j < n; j++) {
        fq_mvpoly_copy(&(*new_matrix)[0][j], &(*original_matrix)[0][j]);
    }
    fprintf(stderr, "[DEBUG] First row copied\n");
    fflush(stderr);
    
    fprintf(stderr, "[DEBUG] Starting row operations...\n");
    fflush(stderr);
    for (slong i = 1; i < n; i++) {
        fprintf(stderr, "[DEBUG] Processing row %ld/%ld...\n", i, n-1);
        fflush(stderr);
        for (slong j = 0; j < n; j++) {
            fprintf(stderr, "[DEBUG]   Processing element [%ld][%ld]...\n", i, j);
            fflush(stderr);
            ...
            fprintf(stderr, "[DEBUG]     diff computed, nterms=%ld\n", diff.nterms);
            fflush(stderr);
      
            if (diff.nterms > 0) {
                fprintf(stderr, "[DEBUG]     Calling divide_by_fq_linear_factor_flint...\n");
                fflush(stderr);
                divide_by_fq_linear_factor_flint(&(*new_matrix)[i][j], &diff, 
                                               i-1, 2*nvars, npars);
                fprintf(stderr, "[DEBUG]     divide_by_fq_linear_factor_flint returned\n");
                fflush(stderr);
            } else {
                fprintf(stderr, "[DEBUG]     Initializing zero polynomial...\n");
                fflush(stderr);
                fq_mvpoly_init(&(*new_matrix)[i][j], 2*nvars, npars, diff.ctx);
                fprintf(stderr, "[DEBUG]     Zero polynomial initialized\n");
                fflush(stderr);
            }
            
            fprintf(stderr, "[DEBUG]     Clearing diff...\n");
            fflush(stderr);
            fq_mvpoly_clear(&diff);
            fprintf(stderr, "[DEBUG]     diff cleared\n");
            fflush(stderr);
        }
        fprintf(stderr, "[DEBUG] Row %ld completed\n", i);
        fflush(stderr);
    }
    
    fprintf(stderr, "[DEBUG] perform_fq_matrix_row_operations_mvpoly: END\n");
    fflush(stderr);
}
...
void fq_dixon_resultant_with_names(fq_mvpoly_t *result, fq_mvpoly_t *polys, 
    ...
    fprintf(stderr, "\n[DEBUG] Step 1: Build Dixon polynomial\n");
    clock_t step1_start = clock();
    fq_mvpoly_t **M_mvpoly;
    fprintf(stderr, "[DEBUG] Build Cancellation Matrix\n");
    build_fq_cancellation_matrix_mvpoly(&M_mvpoly, polys, nvars, npars);
    fprintf(stderr, "[DEBUG] Cancellation Matrix built successfully\n");
    ...
    // Debug: Check matrix[0][0] context
    fprintf(stderr, "[DEBUG] Matrix[0][0] context: nvars=%ld, npars=%ld, ctx=%p\n", 
    ...
    fprintf(stderr, "[DEBUG] Extracting coefficient matrix from Dixon polynomial...\n");
    extract_fq_coefficient_matrix_from_dixon(&coeff_matrix, row_indices, col_indices,
                                            &matrix_size, &d_poly, nvars, npars);
    fprintf(stderr, "[DEBUG] Coefficient matrix extracted, size: %ld\n", matrix_size);
    
    if (matrix_size > 0) {
        fprintf(stderr, "\n[DEBUG] Step 4: Compute resultant\n");
        
        slong res_deg_bound = compute_fq_dixon_resultant_degree_bound(polys, nvars+1, nvars, npars);
        fprintf(stderr, "[DEBUG] Degree bound: %ld\n", res_deg_bound);
        ...
        if (dixon_global_method != -1) {
            coeff_method = dixon_global_method;
            fprintf(stderr, "[DEBUG] Method overridden by global variable: %d\n", dixon_global_method);
        }
        fprintf(stderr, "[DEBUG] Using determinant method: %d\n", coeff_method);
        
        fprintf(stderr, "[DEBUG] Computing coefficient matrix determinant...\n");
        compute_fq_coefficient_matrix_det(result, coeff_matrix, matrix_size,
                                         npars, polys[0].ctx, coeff_method, res_deg_bound);
        ...
        fprintf(stderr, "[DEBUG] Making resultant monic...\n");
        fq_mvpoly_make_monic(result);
        fprintf(stderr, "[DEBUG] Made resultant monic\n");
        
        fprintf(stderr, "[DEBUG] Cleaning up coefficient matrix...\n");
        for (slong i = 0; i < matrix_size; i++) {
            for (slong j = 0; j < matrix_size; j++) {
                fq_mvpoly_clear(&coeff_matrix[i][j]);
            }
        ...
        flint_free(coeff_matrix);
        fprintf(stderr, "[DEBUG] Coefficient matrix cleaned up\n");
    } else {
        fq_mvpoly_init(result, 0, npars, polys[0].ctx);
        fprintf(stderr, "[DEBUG] Warning: Empty coefficient matrix, resultant is 0\n");
    }
    
    flint_free(row_indices);
    flint_free(col_indices);
    
    fprintf(stderr, "[DEBUG] Clearing Dixon polynomial...\n");
    fq_mvpoly_clear(&d_poly);
    fprintf(stderr, "[DEBUG] Dixon polynomial cleared\n");
    
    fprintf(stderr, "[DEBUG] Cleaning up matrices...\n");
    for (slong i = 0; i <= nvars; i++) {
        for (slong j = 0; j <= nvars; j++) {
            fq_mvpoly_clear(&M_mvpoly[i][j]);
            fq_mvpoly_clear(&modified_M_mvpoly[i][j]);
    ...
    flint_free(M_mvpoly);
    flint_free(modified_M_mvpoly);
    fprintf(stderr, "[DEBUG] Matrices cleaned up\n");

    fprintf(stderr, "\n[DEBUG] === Dixon Resultant Computation Complete ===\n");
}
