...
void nmod_mpoly_to_fq_mvpoly(fq_mvpoly_t *result, const nmod_mpoly_t poly,
    ...
    for (slong i = 0; i < nterms; i++) {
        ...
        // Special handling: if poly->exps is NULL, it's a constant polynomial with all exponents zero
        if (poly->exps == NULL) {
            fprintf(stderr, "[DEBUG] poly->exps is NULL, treating as constant (all exponents zero)\n");
            for (slong j = 0; j < total_vars; j++) {
                exp_buffer[j] = 0;
            }
        } else {
            // Get exponents normally
...
void compute_fq_det_unified_interface(fq_mvpoly_t *result, fq_mvpoly_t **matrix, slong size) {
    ...
    unified_mpoly_ctx_t u_ctx = unified_mpoly_ctx_init(total_vars, ORD_LEX, &field_ctx);
    if (!u_ctx) {
    ...
    for (slong i = 0; i < size; i++) {
        ...
        for (slong j = 0; j < size; j++) {
            unified_matrix[i][j] = unified_mpoly_init(u_ctx);
    ...
    // Step 4.5: Ensure all unified polynomials have consistent bits
    fprintf(stderr, "[DEBUG] Repacking all matrix elements to MPOLY_MIN_BITS\n");
    fflush(stderr);
    for (slong i = 0; i < size; i++) {
        for (slong j = 0; j < size; j++) {
            nmod_mpoly_struct *poly_elem = GET_NMOD_POLY(unified_matrix[i][j]);
            if (poly_elem->bits != MPOLY_MIN_BITS) {
                fprintf(stderr, "[DEBUG] Repacking matrix[%ld][%ld]: bits=%u->%u\n", 
                        i, j, (unsigned)poly_elem->bits, (unsigned)MPOLY_MIN_BITS);
                nmod_mpoly_repack_bits_inplace(poly_elem, MPOLY_MIN_BITS, &(u_ctx->ctx.nmod_ctx));
            }
        }
    }
    fprintf(stderr, "[DEBUG] Done repacking\n");
    fflush(stderr);
    ...
    */
    ...
    fprintf(stderr, "[DEBUG] Calling unified_mpoly_init\n");
    fflush(stderr);
    unified_mpoly_t det_unified = unified_mpoly_init(u_ctx);
    fprintf(stderr, "[DEBUG] unified_mpoly_init returned: %p\n", (void*)det_unified);
    fflush(stderr);
    if (!det_unified) {
    ...
    fprintf(stderr, "[DEBUG] About to call start_timing\n");
    fflush(stderr);
    timing_info_t det_start = start_timing();
    fprintf(stderr, "[DEBUG] start_timing done\n");
    fflush(stderr);
    int use_parallel = (size >= PARALLEL_THRESHOLD && max_threads > 1);
    fprintf(stderr, "[DEBUG] About to call compute_unified_mpoly_det, use_parallel=%d\n", use_parallel);
    fflush(stderr);
    compute_unified_mpoly_det(det_unified, unified_matrix, size, u_ctx, use_parallel);
    fprintf(stderr, "[DEBUG] compute_unified_mpoly_det completed\n");
    fflush(stderr);
    
    // DEBUG: Check det_unified state
    fprintf(stderr, "[DEBUG] det_unified state after compute:\n");
    fflush(stderr);
    nmod_mpoly_struct *det_nmod = GET_NMOD_POLY(det_unified);
    fprintf(stderr, "[DEBUG]   det_nmod->length=%ld\n", (slong)det_nmod->length);
    fflush(stderr);
    fprintf(stderr, "[DEBUG]   det_nmod->bits=%u\n", (unsigned)det_nmod->bits);
    fflush(stderr);
    fprintf(stderr, "[DEBUG]   det_nmod->coeffs=%p, det_nmod->exps=%p\n", (void*)det_nmod->coeffs, (void*)det_nmod->exps);
    fflush(stderr);
    fprintf(stderr, "[DEBUG] About to call end_timing\n");
    fflush(stderr);
    timing_info_t det_elapsed = end_timing(det_start);
    fprintf(stderr, "[DEBUG] end_timing done\n");
    fflush(stderr);
    //print_timing("Determinant computation (unified)", det_elapsed);

    fprintf(stderr, "[DEBUG] About to call unified_mpoly_length\n");
    fflush(stderr);
    DET_PRINT("Unified determinant has %ld terms\n", unified_mpoly_length(det_unified));
    fprintf(stderr, "[DEBUG] unified_mpoly_length done\n");
    fflush(stderr);
    // Step 7: Convert result back to fq_mvpoly (with debugging)
    fprintf(stderr, "[DEBUG] About to call start_timing for result\n");
    fflush(stderr);
    timing_info_t result_start = start_timing();
    fprintf(stderr, "[DEBUG] start_timing for result done\n");
    fflush(stderr);
    fprintf(stderr, "[DEBUG] About to call fq_mvpoly_clear\n");
    fflush(stderr);
    fq_mvpoly_clear(result);  // Clear the initialization from the beginning
    fprintf(stderr, "[DEBUG] fq_mvpoly_clear done\n");
    fflush(stderr);
    fprintf(stderr, "[DEBUG] About to call fq_mvpoly_init\n");
    fflush(stderr);
    fq_mvpoly_init(result, nvars, npars, ctx);
    fprintf(stderr, "[DEBUG] fq_mvpoly_init done\n");
    fflush(stderr);
   ...
   // printf("\n--- Converting result back to fq_mvpoly ---\n");
    ...
    fprintf(stderr, "[DEBUG] Checking field type, field_id=%d\n", field_ctx.field_id);
    fflush(stderr);
    if (field_ctx.field_id == FIELD_ID_NMOD || is_prime_field(ctx)) {
        fprintf(stderr, "[DEBUG] Using prime field result conversion\n");
        fflush(stderr);
        //printf("Using prime field result conversion\n");
        // For prime fields, convert from nmod_mpoly
        fprintf(stderr, "[DEBUG] About to call GET_NMOD_POLY\n");
        fflush(stderr);
        nmod_mpoly_struct *nmod_poly = GET_NMOD_POLY(det_unified);
        fprintf(stderr, "[DEBUG] GET_NMOD_POLY returned: %p\n", (void*)nmod_poly);
        fflush(stderr);
        fprintf(stderr, "[DEBUG] About to access u_ctx->ctx.nmod_ctx\n");
        fflush(stderr);
        nmod_mpoly_ctx_struct *nmod_ctx = &(u_ctx->ctx.nmod_ctx);
        fprintf(stderr, "[DEBUG] nmod_ctx: %p\n", (void*)nmod_ctx);
        fflush(stderr);
        
        // Step 0.5: Ensure result polynomial has valid bits and exps
        if (nmod_poly->bits == 0 || nmod_poly->exps == NULL) {
            fprintf(stderr, "[DEBUG] Result poly has invalid state - repacking\n");
            fflush(stderr);
            if (nmod_poly->bits == 0) {
                nmod_poly->bits = MPOLY_MIN_BITS;
            }
            nmod_mpoly_repack_bits_inplace(nmod_poly, MPOLY_MIN_BITS, nmod_ctx);
            fprintf(stderr, "[DEBUG] After repacking: bits=%u, exps=%p\n", 
                    (unsigned)nmod_poly->bits, (void*)nmod_poly->exps);
            fflush(stderr);
        }

        // Use the existing conversion function
        fprintf(stderr, "[DEBUG] About to call nmod_mpoly_length\n");
        fflush(stderr);
        slong nterms = nmod_mpoly_length(nmod_poly, nmod_ctx);
        fprintf(stderr, "[DEBUG] nmod_mpoly_length returned: %ld\n", nterms);
        fflush(stderr);
        fprintf(stderr, "[DEBUG] result->alloc = %ld, nterms = %ld\n", result->alloc, nterms);
        fflush(stderr);
        DET_PRINT("Converting nmod_mpoly with %ld terms\n", nterms);

        // Pre-allocate the result polynomial to avoid reallocations
        if (result->alloc < nterms) {
            fprintf(stderr, "[DEBUG] About to call flint_realloc\n");
            fflush(stderr);
            result->alloc = nterms + nterms/10; // Add 10% extra space
            result->terms = (fq_monomial_t*) flint_realloc(result->terms, 
                                                            result->alloc * sizeof(fq_monomial_t));
            fprintf(stderr, "[DEBUG] flint_realloc done, result->terms = %p\n", (void*)result->terms);
            fflush(stderr);
        }

        // Allocate a temporary exponent array once
        fprintf(stderr, "[DEBUG] About to call flint_malloc for exp_ui, total_vars = %ld\n", total_vars);
        fflush(stderr);
        ulong *exp_ui = (ulong*) flint_malloc(total_vars * sizeof(ulong));
        fprintf(stderr, "[DEBUG] flint_malloc done, exp_ui = %p\n", (void*)exp_ui);
        fflush(stderr);

        // Batch convert all terms
        fprintf(stderr, "[DEBUG] Starting term conversion loop, nterms = %ld\n", nterms);
        fflush(stderr);
        result->nterms = 0;
        for (slong i = 0; i < nterms; i++) {
            fprintf(stderr, "[DEBUG] Converting term %ld/%ld\n", i, nterms);
            fflush(stderr);
            // Get coefficient
            fprintf(stderr, "[DEBUG] About to call nmod_mpoly_get_term_coeff_ui\n");
            fflush(stderr);
            mp_limb_t coeff_ui = nmod_mpoly_get_term_coeff_ui(nmod_poly, i, nmod_ctx);
            fprintf(stderr, "[DEBUG] nmod_mpoly_get_term_coeff_ui returned: %lu\n", coeff_ui);
            fflush(stderr);
            //printf("Result term %ld: coeff_ui = %lu\n", i, coeff_ui);

            // Get exponents
            fprintf(stderr, "[DEBUG] About to get exponents\n");
            fflush(stderr);
            fprintf(stderr, "[DEBUG] exp_ui=%p, nmod_poly=%p, i=%ld, nmod_ctx=%p\n", (void*)exp_ui, (void*)nmod_poly, i, (void*)nmod_ctx);
            fflush(stderr);
            fprintf(stderr, "[DEBUG] nmod_poly->length=%ld\n", (slong)nmod_poly->length);
            fflush(stderr);
            fprintf(stderr, "[DEBUG] nmod_poly->bits=%u\n", (unsigned)nmod_poly->bits);
            fflush(stderr);
            fprintf(stderr, "[DEBUG] nmod_poly->coeffs=%p, nmod_poly->exps=%p\n", (void*)nmod_poly->coeffs, (void*)nmod_poly->exps);
            fflush(stderr);
            fprintf(stderr, "[DEBUG] nmod_ctx=%p, &nmod_ctx->minfo=%p, nmod_ctx->minfo.nvars=%ld\n", 
                    (void*)nmod_ctx, (void*)&nmod_ctx->minfo, nmod_ctx->minfo[0].nvars);
            fflush(stderr);
            
            if (nmod_poly->exps == NULL) {
                // Special case: exps is NULL, assume it's y (second variable, index 1)
                fprintf(stderr, "[DEBUG] exps is NULL, manually setting to y\n");
                fflush(stderr);
                for (slong k = 0; k < total_vars; k++) {
                    exp_ui[k] = (k == 1) ? 1 : 0; // y is the second variable (index 1)
                }
            } else {
                fprintf(stderr, "[DEBUG] About to call nmod_mpoly_get_term_exp_ui\n");
                fflush(stderr);
                nmod_mpoly_get_term_exp_ui(exp_ui, nmod_poly, i, nmod_ctx);
                fprintf(stderr, "[DEBUG] nmod_mpoly_get_term_exp_ui done\n");
                fflush(stderr);
            }
            /*
            printf("  exponents: ");
        ...
        fq_zech_mpoly_ctx_struct *zech_ctx = &(u_ctx->ctx.zech_ctx);
        ...
        fq_nmod_mpoly_ctx_struct *fq_ctx = &(u_ctx->ctx.fq_ctx);
