...
void divide_by_fq_linear_factor_flint(fq_mvpoly_t *quotient, const fq_mvpoly_t *dividend,
                                     slong var_idx, slong nvars, slong npars) {
    fprintf(stderr, "[DEBUG] divide_by_fq_linear_factor_flint: START var_idx=%ld, nvars=%ld, npars=%ld\n", var_idx, nvars, npars);
    fflush(stderr);
    
    fprintf(stderr, "[DEBUG]   dividend->nterms=%ld\n", dividend->nterms);
    fflush(stderr);
    
    fq_mvpoly_init(quotient, nvars, npars, dividend->ctx);
    
    if (dividend->nterms == 0) {
        fprintf(stderr, "[DEBUG] divide_by_fq_linear_factor_flint: END (empty dividend)\n");
        fflush(stderr);
        return;
    }
    
    slong total_vars = nvars + npars;
    fprintf(stderr, "[DEBUG]   total_vars=%ld\n", total_vars);
    fflush(stderr);
    
    fprintf(stderr, "[DEBUG]   Initializing mpoly_ctx...\n");
    fflush(stderr);
    fq_nmod_mpoly_ctx_t mpoly_ctx;
    fq_nmod_mpoly_ctx_init(mpoly_ctx, total_vars, ORD_LEX, dividend->ctx);
    fprintf(stderr, "[DEBUG]   mpoly_ctx initialized\n");
    fflush(stderr);
    
    fprintf(stderr, "[DEBUG]   Converting dividend to mpoly...\n");
    fflush(stderr);
    ...
    fprintf(stderr, "[DEBUG]   Dividend converted\n");
    fflush(stderr);
    
    fprintf(stderr, "[DEBUG]   Building divisor...\n");
    fflush(stderr);
    ...
    fprintf(stderr, "[DEBUG]   Divisor built\n");
    fflush(stderr);
    
    fprintf(stderr, "[DEBUG]   Skipping divides check, using divrem directly...\n");
    fflush(stderr);
    ...
    fprintf(stderr, "[DEBUG]   Calling fq_nmod_mpoly_divrem...\n");
    fflush(stderr);
    fq_nmod_mpoly_divrem(quotient_mpoly, remainder, dividend_mpoly, divisor, mpoly_ctx);
    fprintf(stderr, "[DEBUG]   fq_nmod_mpoly_divrem returned\n");
    fflush(stderr);
    
    if (fq_nmod_mpoly_is_zero(remainder, mpoly_ctx)) {
        fprintf(stderr, "[DEBUG]   Remainder is zero, converting result...\n");
        fflush(stderr);
        fq_mvpoly_clear(quotient);
        fq_nmod_mpoly_to_fq_mvpoly(quotient, quotient_mpoly, nvars, npars, mpoly_ctx, dividend->ctx);
        fprintf(stderr, "[DEBUG]   Result converted\n");
        fflush(stderr);
    } else {
        fprintf(stderr, "[DEBUG]   Warning: FLINT division not exact, falling back to direct method\n");
        fflush(stderr);
        fq_mvpoly_clear(quotient);
    ...
    fprintf(stderr, "[DEBUG] divide_by_fq_linear_factor_flint: END\n");
    fflush(stderr);
}
